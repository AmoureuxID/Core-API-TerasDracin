...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== "GET") {
    ...
    res.status(405).setHeader("Content-Type", "application/json; charset=utf-8").json(body);
    return;
  }
  ...
  if (heavy && !isGuardAllowed(req)) {
    ...
    res.status(403).setHeader("Content-Type", "application/json; charset=utf-8").json(body);
    return;
  }
  ...
  try {
    ...
    const body: ApiError = { status: false, message, data: null };
    res.status(504).setHeader("Content-Type", "application/json; charset=utf-8").json(body);
  } finally {
